...
An instance of this class is created based on input parameters of the ECG data
and a reference waveform. The functions in this class allows for the
computation of the mean HR, max/min voltages, duration, number beats and
corresponding times for each beat. There are also input checking, float
variable checking and overall output functions.
"""
...
class HeartRateMonitor:
    ...
    def __init__(self, time=[], voltage=[], voltage_ref=[], time_unit="sec",
                 filename=""):
        """Initializes instance of HeartRateMonitor based on file input and
        sets up logging

        :param time:
        :param voltage:
        :param voltage_ref:
        :param time_unit: users can input unit for timescale, if none
        specified, unit is assumed to be seconds
        """
    ...
    def is_float(self, input):
        ...
        :return: boolean value of whether input is numerical
        """
        try:
            float(input)
            return True
    ...
    def process_input(self):
        """Checks that the input is the right type and that the time and
        voltage vectors are of equal length
        ...
        try:
            ...
            for idx, i in enumerate(self.time_input):
                if self.is_float(i):
                    ...
                    if self.is_float(self.time_input[idx-1]) and \
                            self.is_float(self.time_input[idx+1]):
                        fill_in = (float(self.time_input[idx-1]) +
                                   float(self.time_input[idx+1]))/2
                    else:
                        fill_in = self.time[len(self.time) - 1]
                    self.time.append(fill_in)
                    logging.warning("Time gap detected. Interpolation "
                                    "completed.")
        except TypeError:
            logging.error("There is a TypeError. First time entry must be a "
                          "numerical value.")
        ...
        try:
            for idx, j in enumerate(self.voltage_input):
                if self.is_float(j):
                    ...
                    if self.is_float(self.voltage_input[idx-1]) and \
                            self.is_float(self.voltage_input[idx+1]):
                        fill_in = (float(self.voltage_input[idx-1]) +
                                   float(self.voltage_input[idx+1]))/2
                    else:
                        fill_in = self.voltage[len(self.voltage) - 1]
                    self.voltage.append(fill_in)
                    logging.warning("Bad data in voltage detected. "
                                    "Interpolation completed.")
        except TypeError:
            logging.error("There is a TypeError. First voltage entry must be "
                          "a numerical value.")
        ...
        try:
            ...
            logging.error("There is a ValueError in the list of times. The "
        ...
        try:
            ...
            logging.error("There is a ValueError in the list of voltages. The "
                          "numbers must be real.")
            print("Voltage input must be a list containing only of real "
                  "numbers.")
    ...
    def mean_hr_bpm(self, num_beats=None):
        """Calculates the mean heart rate in beats per minute.

        :param num_beats: number of beats per minute can be manually inputted,
        if not it runs num_beats()
        ...
        mean_hr_bpm = num_beats/duration_min

        logging.info("Mean heart rate successfully calculated.")
        if mean_hr_bpm > 180:
            logging.warning("Warning: Mean heart rate detected to be more "
                            "than 180 bpm.")
        if mean_hr_bpm < 40:
            logging.warning("Warning: Mean heart rate detected to be less "
                            "than 40 bpm.")
    ...
    def voltage_extremes(self):
        ...
        if abs(voltage_extremes[0]) > 300 or abs(voltage_extremes[1]) > 300:
            logging.warning("Warning: Voltage is out of ECG range (>300 mV)")
    ...
    def duration(self):
        ...
        if duration < 5:
            logging.info("Warning: Duration is less than 5 seconds. "
                         "Calculation accuracy may be affected.")
    ...
    def num_beats(self):
        ...
        corr_voltage = numpy.convolve(norm_voltage, norm_voltage_ref[144:504],
                                      mode='same')
    ...
    def beats(self):
        ...
        logging.info("Times corresponding to each heartbeat successfully "
                     "identified.")
